...
def define_discriminator(n_classes, opt, in_shape=(1, 256, 1)):
    ...
    fe = Conv2D(filters=32, kernel_size=(1,25), strides=2)(inp)
    fe = LeakyReLU()(fe)
    #fe = ReLU()(fe)
...
def define_trans_discriminator(n_classes, opt, in_shape=(1, 256, 1)):
    inp = Input(shape= in_shape)
    #li = Reshape((1, 120, 1))(inp)
    fe = Conv2D(filters=32, kernel_size=(1,25), strides=2)(inp)
    fe = LeakyReLU()(fe)
...
def define_trans_generator(latent_dim = 100):
    in_lat = Input(shape=(latent_dim,))
    n_nodes = 32 * 1 * 244
    gen = Dense(n_nodes)(in_lat)
    gen = ReLU()(gen)
...
def define_trans_generator_for_case2(latent_dim = 100):
    in_lat = Input(shape=(latent_dim,))
    n_nodes = 32 * 1 * 244
    gen = Dense(n_nodes)(in_lat)
    gen = ReLU()(gen)
...
#     model = Sequential()
#     model.add(Conv2D(filters=32, kernel_size=(1,5), strides=1, input_shape=(1, 256, 1), activation='relu')) # 256 -4 -4 -4 = 244
...
def define_trans_discriminator_test(n_classes, opt, in_shape=(1, 256, 1)):
    inp = Input(shape= in_shape)
    #li = Reshape((1, 120, 1))(inp)
    fe = Conv2D(filters=32, kernel_size=(1,25), strides=2)(inp)
    fe = LeakyReLU()(fe)
...
def define_trans_generator_test(latent_dim = 100):
    in_lat = Input(shape=(latent_dim,))
    n_nodes = 32 * 1 * 244
    gen = Dense(n_nodes)(in_lat)
    gen = ReLU()(gen)
