...
# 122번째 줄에서 레이블이 1인 경우에만 train 데이터에 추가하도록 수정
# 클래스(레이블) 수 / 클래스(레이블) 마다 파일이 하나씩 있음
class_num = 3
# 각 파일 당 최대 데이터 수
max_per_file = 1250
# 파일 경로 패턴 설정
pcap_base_path = "/home/mnetlig/Desktop/CSI-SemiGAN-master/dataset/c_{}.pcap" # 수집한 pcap 파일의 경로 입력
...
# 새로운 확장자 추가
output_file_name = f"Removed_{file_name_without_ext}.pickle"
...
for i in range(1, class_num+1):
    ...
    with open(save_path, 'rb') as f:
        data = pickle.load(f)
        merged_data[0].extend(data[0])  # 1열 (400, 256) 형태의 2차원 리스트ㅋ
...
for j in range(len(merged_data[0])):
    # if merged_data[1][j] % 2 != 0:  # 레이블이 홀수인 경우에만 추가
    if merged_data[1][j] == 0 or merged_data[1][j] == 2:  # 레이블이 0 또는 2인 경우에만 추가
        pairs_12.append((merged_data[0][j], merged_data[1][j]))

# 평가데이터는 모든 레이블에 대해 사용
